...
if not os.path.exists('midi_data_from_jams'):
    os.mkdir('midi_data_from_jams')
...
for jams_file in jams_files:
    ...
    jams_path = os.path.join('jams_data', jams_file)
    midi_file_name = os.path.splitext(jams_file)[0] + '.mid'
    midi_file_path = os.path.join('midi_data_from_jams', midi_file_name)
